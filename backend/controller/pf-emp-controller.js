...
const pfEmpInfo = require("../model/pf-emp-model");
const payrollUserModel = require("../model/payroll-user-model");
...
exports.createPfEmpInfo = catchAysncError(async (req, res, next) => {
  ...
  const payrollIds = await payrollUserModel.find(
    { empStatus: "Confirmed" },
    { _id: 0, empId: 1 }
  );
  ...
  if (notUpdated.length > 0) {
    return res
      ...
      .json({
        success: true,
        error: `${notUpdated} not added yet in ERP system`,
      });
  }
